...
real*8 :: Amp = 0.22,kf=56.,ptimestep= 3805.2,radius = 58232000 ! ! ptimestep = 1189.125 ; 3805.2 (itau_physics=10 ; 32) ; Rayon en metre Jupiter : radius = 71492000
integer :: TGforcage = 1,NetcdefStatData=1  
!**********************************************************
integer :: iarg,narg,mt,istep,getfield,tstep,dimid_out(3),varid_out(4),latid,altid,colatid,lonid,tid,idinfos,ninfos,idninfos,Urmsid,hnid,fnid,dstepsid,epsilonfid,ERid,ETid,EZid,hn_zmid,fn_zmid,Enid,En_divid,En_rotid,En_zmid,fnrotid,hnrotid,fndiv1id,hndiv1id,fndiv2id,hndiv2id
...
double precision, dimension (:), allocatable :: fn_zm,hn_zm,fn_rot,hn_rot,fn_div1,hn_div1,fn_div2,hn_div2,Enmo_zm,Ene_zm,Urms
double precision, dimension(:,:,:), allocatable :: vort_r, vort_i, div_r, div_i, sf_r, sf_i, vp_r, vp_i
double precision, dimension(:,:), allocatable :: vortf_r, vortf_i, divf_r, divf_i,Enmo,Ene,En_rot,En_div
double precision, dimension(:,:,:), allocatable :: vort,term1_r,term1_i,term2_v,term2_w, term3,vort_uv_br,vort_uv_bi,vort_uv_cr,vort_uv_ci,vortv,vortu,uv_dot_grad_vort_r,uv_dot_grad_vort_i,rot_ugradu_i,rot_ugradu_r,div1_ugradu_i,div1_ugradu_r,fmn,hmn,Emn
...
! champ 1d
 ! call check( NF90_PUT_ATT  (ncidF, En_zmid, "_FillValue", NF90_FILL_DOUBLE) )		!--------------------------temporellement suprime
 ! call check( NF90_PUT_ATT  (ncidF, Enid, "_FillValue", NF90_FILL_DOUBLE) )		!--------------------------temporellement suprime
...
! call DIVROTV(nlon,nlat,nalt,isym,v_rot,u_rot,v_div,u_div,mdab_s,ndab,vort_r,vort_i,div_r,div_i) !--------------------RECONNECTER
v_rot = v_rot * radius
u_rot = u_rot * radius
v_div = v_div * radius
u_div = u_div * radius
...
allocate(Ene_zm(ndab-1)) ; Ene_zm    = 0.0d0
 call ES(mdab_s,ndab,nalt,radius,vort_r,vort_i,div_r,div_i,Emn,Enmo,Ene,Enmo_zm,Ene_zm,ER,EZ,ET,En_rot,En_div)			!--------------------------temporellement suprime
!########################################################################################################################################
!========================================================================================================================================
...
! Vector Spectral Analysis
 !call VSA(nlon,nlat,nalt,ityp,mdab_v,ndab,vortv,vortu,vort_uv_br,vort_uv_bi,vort_uv_cr,vort_uv_ci) !--------------------RECONNECTER
!======================================================
...
!do n = 1, ndab-2 								!--------------------RECONNECTER

  ! Scaling factor
  ! Need to divide through by radius as gradient operator is on unit sphere  
 ! factor2 = sqrt(1.0d0 * n * (n+1.0d0)) / radius 				!--------------------RECONNECTER

  ! Divergence spectrum: Spherepack 2.0 after Eq. 4.16
 ! uv_dot_grad_vort_r(1:mdab_smaller,n+1,1:nalt) = -vort_uv_br(1:mdab_smaller,n+1,1:nalt) * factor2 !--------------------RECONNECTER
 ! uv_dot_grad_vort_i(1:mdab_smaller,n+1,1:nalt) = -vort_uv_bi(1:mdab_smaller,n+1,1:nalt) * factor2 !--------------------RECONNECTER
...
allocate(hmn(mdab_s,ndab,nalt)) ; hmn = 0.0d0
 !call FLUXES(mdab_s,ndab,nalt,radius,term1_r,term1_i,uv_dot_grad_vort_r,uv_dot_grad_vort_i,fn,hn,fn_zm,hn_zm,fmn,hmn) !--------------------RECONNECTER
...
!champ 1d
  ! call check( nf90_put_var( ncidF, En_zmid, Enmo,(/1,1,it/),(/nlat,nalt,1/) ) )			!--------------------------temporellement suprime
  ! call check( nf90_put_var( ncidF, Enid, Ene,(/1,1,it/),(/nlat,nalt,1/) ) )				!--------------------------temporellement suprime
