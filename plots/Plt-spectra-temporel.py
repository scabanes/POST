...
ToOMEGA = omega_sat*2.
print omega_sat*2
#plt.loglog(w,FFT[ni,:,:], '-', color=COLORS[1], linewidth=1.)#, alpha=ALPHAS	[il])
ic = -1
...
plt.loglog(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[1], linewidth=1.)#, alpha=ALPHAS	[il])
...
if(0==0):
	...
	ax = plt.subplot(331)	
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.semilogx(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[1], linewidth=1.)#, alpha=ALPHAS	[il])
...
if(0==0):
	...
	ax = plt.subplot(111)
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.semilogx(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[0], linewidth=1.)#, alpha=ALPHAS	[il])
	...
	ni = ni+1
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.semilogx(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[2], linewidth=1.)#, alpha=ALPHAS	[il])
	plt.axvline(omega_RHW[ni], color=COLORS[2], linestyle='dashed', linewidth=1.,label='n = '+str(ni))
	ni = ni+1
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.semilogx(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[3], linewidth=1.)#, alpha=ALPHAS	[il])
	plt.axvline(omega_RHW[ni], color=COLORS[3], linestyle='dashed', linewidth=1.,label='n = '+str(ni))
	ni = ni+1
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.semilogx(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[4], linewidth=1.)#, alpha=ALPHAS	[il])
	plt.axvline(omega_RHW[ni], color=COLORS[4], linestyle='dashed', linewidth=1.,label='n = '+str(ni))
	ni = ni+1
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.semilogx(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[5], linewidth=1.)#, alpha=ALPHAS	[il])
...
if(0==0):
	...
	ax = plt.subplot(111)
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.loglog(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[0], linewidth=1.)#, alpha=ALPHAS	[il])
	plt.axvline(omega_RHW[ni], color=COLORS[0], linestyle='dashed', linewidth=1.,label='n = '+str(ni))
	ni = ni+1
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.loglog(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[1], linewidth=1.)#, alpha=ALPHAS	[il])
	...
	ni = ni+1
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.loglog(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[2], linewidth=1.)#, alpha=ALPHAS	[il])
	plt.axvline(omega_RHW[ni], color=COLORS[2], linestyle='dashed', linewidth=1.,label='n = '+str(ni))
	ni = ni+1
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.loglog(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[3], linewidth=1.)#, alpha=ALPHAS	[il])
	plt.axvline(omega_RHW[ni], color=COLORS[3], linestyle='dashed', linewidth=1.,label='n = '+str(ni))
	ni = ni+1
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.loglog(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[4], linewidth=1.)#, alpha=ALPHAS	[il])
	plt.axvline(omega_RHW[ni], color=COLORS[4], linestyle='dashed', linewidth=1.,label='n = '+str(ni))
	ni = ni+1
	Cst = float(ni)*(float(ni)+1.)/4.
	plt.loglog(w,Cst*np.mean(FFT[ni,:,:],1), '-', color=COLORS[5], linewidth=1.)#, alpha=ALPHAS	[il])
	plt.axvline(omega_RHW[ni], color=COLORS[5], linestyle='dashed', linewidth=1.,label='n = '+str(ni))
#	plt.axvline(omega_RHW[ni], color=COLORS[1], linestyle='dashed', linewidth=1.)
	plt.axvline(ToOMEGA, color=COLORS[2], linestyle='solid', linewidth=1.,label='$2 \Omega$')
	plt.title('m = '+str(im))
